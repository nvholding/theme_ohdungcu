				...
				<tr id="{id}">
					...
					<td align="center">
                        <div class="select-item-qty-mobile">					
							...
							<button onclick="var result = document.getElementById('pnum{id}'); var pnum{id} = result.value; if( !isNaN(pnum{id}) &amp; pnum{id} > 1 ) result.value--;return false;" class="btn-minus btn-cts" type="button">-</button>
							<input type="text" class="qty input-text" id="pnum{id}" name="listproid[{id}]" size="4" value="{pro_num}"/>
							<button onclick="var result = document.getElementById('pnum{id}'); var pnum{id} = result.value; if( !isNaN(pnum{id})) result.value++;return false;" class="btn-plus btn-cts" type="button">+</button>									
							</div>
						</div>
