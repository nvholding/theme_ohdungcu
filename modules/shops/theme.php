 ...
 * @return
 */
function view_home_cat($data_content, $compare_id, $html_pages = '', $sort = 0, $data_review = array())
{
    global $module_info, $lang_module, $module_file, $module_name, $pro_config, $array_wishlist_id, $global_array_shops_cat, $global_array_group, $my_head, $module_upload;
    ...
    if (!empty($data_content)) {
        ...
        foreach ($data_content as $data_row) {
            if ($data_row['num_pro'] > 0) {
                $xtpl->assign('TITLE_CATALOG', $data_row['title']);
                $xtpl->assign('LINK_CATALOG', $data_row['link']);
                $xtpl->assign('ALIAS_CATALOG', $data_row['alias']);
                $xtpl->assign('NUM_PRO', $data_row['num_pro']);
                $xtpl->assign('STT_CATALOG', $stt++);

                if (!empty($data_row['image'])) {
                    $xtpl->assign('IMAGES_CATALOG', NV_BASE_SITEURL . NV_UPLOADS_DIR . '/' . $module_upload . '/' . $data_row['image']);
                    $xtpl->parse('main.catalogs.imagecat');
                }
                ...
                foreach ($data_row['data'] as $data_row_i) {
                    ...
                    if (!empty($pro_config['review_active'])) {
                        $review_star = nv_review_star($data_row_i['id'], $data_review);
                        $xtpl->assign('review_star', $review_star);
                        $xtpl->parse('main.catalogs.items.review');
                    }
					...
					if($data_row_i['has_gift_info']) {
						$xtpl->assign('IMG_SRC_GIFT', NV_BASE_SITEURL . NV_UPLOADS_DIR . '/gift.png');
						$xtpl->parse('main.catalogs.items.gift_info');
					}
					
					if($data_row_i['has_mydiscount']) {
						$xtpl->assign('IMG_SRC_DISCOUNT', NV_BASE_SITEURL . NV_UPLOADS_DIR . '/discount.png');
						$xtpl->parse('main.catalogs.items.mydiscount');
					}
 ...
 * @return
 */
function view_search_all($data_content, $compare_id, $html_pages = '', $data_review = array())
{
    ...
    if (!empty($data_content)) {
        ...
        foreach ($data_content as $data_row) {
            ...
            if (!empty($pro_config['review_active'])) {
                $review_star = nv_review_star($data_row['id'], $data_review);
                $xtpl->assign('review_star', $review_star);
                $xtpl->parse('main.items.review');
            }
 ...
 * @return
 */
function viewcat_page_gird($data_content, $compare_id, $pages, $sort = 0, $viewtype, $data_review = array())
{
    ...
    if ($pro_config['show_displays'] == 1) {
        ...
        $array_viewtype = array('viewcat_page_list' => 'fa fa-th-list', 'viewcat_page_gird' => 'fa fa-th');
        foreach ($array_viewtype as $k => $array_viewtype_i) {
            $se = $k == $viewtype ? ' active' : '';
    ...
    if (!empty($data_content['data'])) {
        ...
        foreach ($data_content['data'] as $data_row) {
            ...
            if (!empty($pro_config['review_active'])) {
                ...
                $xtpl->parse('main.grid_rows.review');
            }
...
function nv_review_star($id_item, $data_review)
{
    ...
    $numstar = $sum_star = 0;
    if (!empty($pro_config['review_active'])) {
        if (!empty($data_review[$id_item])) {
            foreach ($data_review[$id_item] as $review) {
                $sum_star += $review;
            }

            $numstar = ceil($sum_star / sizeof($data_review[$id_item]));
        }


        $xtpl->assign('num_star', $numstar);
    }

    $xtpl->parse('rating');
    return $xtpl->text('rating');
}
 ...
 * @return
 */
function detail_product($data_review = array(), $data_content, $data_unit, $data_others, $array_other_view, $content_comment, $compare_id, $popup, $idtemplate, $array_keyword)
{
    global $module_info, $lang_module, $module_file, $module_name, $module_upload, $pro_config, $global_config, $global_array_group, $array_wishlist_id, $client_info, $global_array_shops_cat, $meta_property, $pro_config, $user_info, $discounts_config, $my_head, $my_footer;
    ...
    if (!empty($data_content)) {
        ...
        $review_star = nv_review_star($data_content['id'], $data_review);
        ...
        if (!empty($data_content['otherimage'])) {
            $otherimage = nv_display_othersimage($data_content['otherimage']);
            $xtpl->assign('PRODUCT_OTHER_IMAGES', $otherimage);
			...
			if (!empty($otherimage)) {
				...
				foreach ($otherimage as $otherimage_i) {
					if (!empty($otherimage_i) and file_exists(NV_UPLOADS_REAL_DIR . '/' . $module_upload . '/' . $otherimage_i)) {
						...
						$xtpl->parse('main.my_image.loop');
					}
				}
				$xtpl->parse('main.my_image');
			}
        ...
        if (!$popup) {
            ...
            if (!empty($data_others)) {
                foreach ($data_others as $others) {
                    $price_others = nv_get_price($others['id'], $pro_config['money_unit']);

                    if ($pro_config['active_price'] == '1') {
                        if ($others['showprice'] == '1') {
                            $xtpl->assign('PRICEOTHER', $price_others);
                            if ($others['discount_id'] and $price_others['discount_percent'] > 0) {
                                $xtpl->parse('main.product_detail.other.loop.price.discounts');
                                $xtpl->parse('main.product_detail.other.loop.price.discounts.standard');
                            } else {
                                $xtpl->parse('main.product_detail.other.loop.price.no_discounts');
                            }
                            $xtpl->parse('main.product_detail.other.loop.price');
                        } else {
                            $xtpl->parse('main.product_detail.other.loop.contact');
                        }
                    }

                    if (!empty($pro_config['review_active'])) {
                        $review_star = nv_review_star($others['id'], $data_review);
                        $xtpl->assign('review_star', $review_star);

                        $xtpl->parse('main.product_detail.other.loop.review');
                    }

                    $xtpl->assign('OTHER', $others);
                    $xtpl->parse('main.product_detail.other.loop');
                }
...
function cart_product($data_content, $coupons_code, $order_info, $array_error_number)
{
    ...
    if (!empty($data_content)) {
        ...
        foreach ($data_content as $data_row) {
            ...
            $xtpl->assign('gift_info', nv_get_gift_info($data_row['id'], $data_row['num']));
                ...
                for ($i = 0; $i < count($array_group_main); $i++) {
                    foreach ($array_sub_group as $sub_group_id) {
                        ...
                        if ($item['parentid'] == $group_main_id) {
                            $data = array('title' => $item['title'], 'link' => $item['link']);
...
function payment($data_content, $data_pro, $data_shipping, $url_checkout, $intro_pay, $point)
{
    ...
    foreach ($data_pro as $pdata) {
        ...
        $xtpl->assign('gift_info', nv_get_gift_info($pdata['id'], $pdata['product_number']));
