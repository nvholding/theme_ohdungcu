...
if( ! nv_function_exists( 'nv_relates_product' ) )
{
    ...
    function nv_relates_product( $block_config )
    {
        ...
        foreach( $list as $row )
        {
            $array_id_item[] = $row['id'];
        }
        
        //rating data
        if( ! empty( $array_id_item ) )
        {
            $db->sqlreset()->select( 'rating, product_id' )->from( $db_config['prefix'] . '_' . $mod_data . '_review t1' )->where( 't1.status = 1 AND t1.product_id IN (' . implode( ',', $array_id_item ) . ')' );
        ...
        foreach( $list as $row )
        {
			...
			$discount_num_pro = $row['discount_num_pro'] ? json_decode($row['discount_num_pro'], true) : array();
			$has_mydiscount = $row['discount_member_amount'] > 0 || (is_array($discount_num_pro) && count($discount_num_pro) > 0);

			$gift_info = $row['gift_info'] ? json_decode($row['gift_info'], true) : array();
            ...
            $newday = $row['publtime'] + ( 86400 * $global_array_shops_cat[$row['listcatid']]['newday'] );
			...
			if($has_gift_info) {
				...
				$xtpl->parse('main.loop.gift_info');
			}

			if($has_mydiscount) {
				$xtpl->assign('IMG_SRC_DISCOUNT', NV_BASE_SITEURL . NV_UPLOADS_DIR . '/discount.png');
				$xtpl->parse('main.loop.mydiscount');
			}
            
            if( ! empty( $pro_config['review_active'] ) )
            {
                $review_star = nv_review_star( $row['id'], $data_review );
                $xtpl->assign( 'review_star', $review_star );
                $xtpl->parse( 'main.loop.review' );
            }
