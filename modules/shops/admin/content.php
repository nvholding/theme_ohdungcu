...
$rowcontent = array(
    ...
    'gift_to' => 0,
    'discount_member_type' => 1,
    'discount_member_amount' => 0,
    'gift_info' => array(),
    'discount_num_pro' => array()
);
...
if ($nv_Request->get_int('save', 'post') == 1) {
    ...
    $rowcontent['discount_member_type'] = $nv_Request->get_int('discount_member_type', 'post', 0);
    $rowcontent['discount_member_amount'] = $nv_Request->get_float('discount_member_amount', 'post', 0);

    $discount_num_pro = $nv_Request->get_typed_array('discount_num_pro', 'post');

    $rowcontent['discount_num_pro'] = [];
    if (is_array($discount_num_pro)) {
        foreach ($discount_num_pro as $dis_num) {
            if (!empty($dis_num['min']) && !empty($dis_num['max']) && !empty($dis_num['amount'])) {
                $rowcontent['discount_num_pro'][] = $dis_num;
            }
        }
    }

    $gift_info = $nv_Request->get_typed_array('gift_info', 'post');
    $rowcontent['gift_info'] = [];
    if ($gift_info) {
        foreach ($gift_info as $iGift) {
            if (!empty($iGift['min']) && !empty($iGift['max']) && !empty($iGift['content'])) {
                $rowcontent['gift_info'][] = $iGift;
            }
        }
    }
    ...
    if (empty($error)) {
        ...
        if ($rowcontent['id'] == 0 or $is_copy) {
            ...
            $sql = "INSERT INTO " . $db_config['prefix'] . "_" . $module_data . "_rows (id, listcatid, user_id, addtime, edittime, status, publtime, exptime, archive, product_code, product_number, product_price, price_config, money_unit, product_unit, product_weight, weight_unit, discount_id, homeimgfile, homeimgthumb, homeimgalt,otherimage,imgposition, copyright, inhome, allowed_comm, allowed_rating, ratingdetail, allowed_send, allowed_print, allowed_save, hitstotal, hitscm, hitslm, showprice, discount_member_amount, discount_member_type, discount_num_pro, gift_info " . $listfield . ")
                 ...
                 " . floatval($rowcontent['discount_member_amount']) . ",
                 " . intval($rowcontent['discount_member_type']) . ",
                 :discount_num_pro,
                 :gift_info
				" . $listvalue . "
			)";
            ...
            $data_insert['discount_num_pro'] = $rowcontent['discount_num_pro'] ? json_encode($rowcontent['discount_num_pro']) : '';
            $data_insert['gift_info'] = $rowcontent['gift_info'] ? json_encode($rowcontent['gift_info']) : '';
            ...
            $stmt = $db->prepare("UPDATE " . $db_config['prefix'] . "_" . $module_data . "_rows SET
			 ...
			 discount_member_amount = " . floatval($rowcontent['discount_member_amount']) . ",
			 discount_member_type = " . intval($rowcontent['discount_member_type']) . ",
			 discount_num_pro = :discount_num_pro,
             gift_info = :gift_info,
            ...
            if (empty($rowcontent['allowed_comm'])) $rowcontent['allowed_comm'] = 0;
            ...
            $discount_num_pro = is_array($rowcontent['discount_num_pro']) ? json_encode($rowcontent['discount_num_pro']) : '';
            $gift_info = is_array($rowcontent['gift_info']) ? json_encode($rowcontent['gift_info']) : '';
            $stmt->bindParam(':discount_num_pro', $discount_num_pro, PDO::PARAM_STR);
            $stmt->bindParam(':gift_info', $gift_info, PDO::PARAM_STR);
    ...
    //////////////////////////////////////
    $discount_num_pro = $rowcontent['discount_num_pro'] ? json_decode($rowcontent['discount_num_pro'], true) : [];
    $rowcontent['discount_num_pro'] = [];
    if (is_array($discount_num_pro)) {
        foreach ($discount_num_pro as $dis_num) {
            if (!empty($dis_num['min']) && !empty($dis_num['max']) && !empty($dis_num['amount'])) {
    ...
    $gift_info = $rowcontent['gift_info'] ? json_decode($rowcontent['gift_info'], true) : [];
    $rowcontent['gift_info'] = [];
    if ($gift_info) {
        foreach ($gift_info as $iGift) {
            if (!empty($iGift['min']) && !empty($iGift['max']) && !empty($iGift['content'])) {
...
// discount_num_pro
$items = 0;
if ($rowcontent['discount_num_pro']) {
    foreach ($rowcontent['discount_num_pro'] as $iDiscount) {
        $iDiscount['k'] = $items;
        $iDiscount['select'] = '<select class="form-control" name="discount_num_pro[' . $items . '][type]">
        <option value="1" ' . ($iDiscount['type'] == 1 ? "selected" : "") . '>Tỉ lệ phần trăm</option>
        <option value="2" ' . ($iDiscount['type'] == 2 ? "selected" : "") . '>Số tiền cố định</option>
        </select>';
        $xtpl->assign('DISCOUNT_NUM_PRO', $iDiscount);
        $xtpl->parse('main.discount_num_pro');
        ++$items;
    }
}
$xtpl->assign('NUM_DISCOUNT_NUM_PRO', $items);
$xtpl->assign('SELECT_DISCOUNT_MEMBER', '<select class="form-control" name="discount_member_type">
<option value="1" ' . ($rowcontent['discount_member_type'] == 1 ? "selected" : "") . '>Tỉ lệ phần trăm</option>
<option value="2" ' . ($rowcontent['discount_member_type'] == 2 ? "selected" : "") . '>Số tiền cố định</option>
</select>');

// gift_info
$items = 0;
if ($rowcontent['gift_info']) {
    foreach ($rowcontent['gift_info'] as $iGift) {
        $iGift['k'] = $items;
        $xtpl->assign('GIFT_INFO', $iGift);
        $xtpl->parse('main.gift_info');
        ++$items;
    }
}
$xtpl->assign('NUM_GIFT_INFO', $items);
