...
function nv_get_gift_info($pro_id, $number = 1)
{
    global $db, $db_config, $module_data;
    $product = $db->query('SELECT gift_info FROM ' . $db_config['prefix'] . '_' . $module_data . '_rows WHERE id = ' . $pro_id)->fetch();

    if ($product['gift_info']) {
        $gift_info = json_decode($product['gift_info'], true);
        if (is_array($gift_info) && count($gift_info) > 0) {
            foreach ($gift_info as $iGift) {
                if ($number >= $iGift['min'] && $number <= $iGift['max']) {
                    return $iGift['content'];
                }
            }
        }
    }
...
{
    global $db, $db_config, $site_mods, $module_data, $global_array_shops_cat, $pro_config, $money_config, $discounts_config, $user_info;
    ...
    $product = $db->query('SELECT listcatid, product_price, money_unit, price_config, discount_id, discount_member_amount, discount_member_type, discount_num_pro FROM ' . $db_config['prefix'] . '_' . $module_data . '_rows WHERE id = ' . $pro_id)->fetch();
    $price = $old_price = $product['product_price'];
	...
	//print_r($price . " - " . $old_price . "<br>");
	if (isset($user_info['userid'])) {
        if ($product['discount_member_amount'] > 0) {
            if ($product['discount_member_type'] == 1) {
                $userDiscount = (($price) * ($product['discount_member_amount'] / 100));
            } else {
                $userDiscount = ($product['discount_member_amount']);
            }
        }
    }
	...
	//print_r($price . " - " . $old_price . "<br>");
    // tính toán 
    if ($product['discount_num_pro']) {
        $discount_num_pro = json_decode($product['discount_num_pro'], true);
        if (is_array($discount_num_pro) && count($discount_num_pro) > 0) {
            foreach ($discount_num_pro as $iDiscount) {
                if ($number >= $iDiscount['min'] && $number <= $iDiscount['max']) {
                    if ($iDiscount['type'] == 1) {
                        $myDiscount = ($price) * ($iDiscount['amount'] / 100);
                    } else {
                        $myDiscount = $iDiscount['amount'];
                    }
                }
            }
        }
    }
    $price = $price - $myDiscount;
    if (!$per_pro) {
        $price = $price * $number;
    }
...
//print_r($price . " - " . $old_price . "<br>");
	$myprice = $discount > 0 ? $old_price : $price;
    $return['price'] = $myprice; // Giá sản phẩm chưa format
    $return['price_format'] = nv_number_format($myprice, $decimals); // Giá sản phẩm đã format
    ...
    $return['sale'] = $price; // Giá bán thực tế của sản phẩm
    $return['sale_format'] = nv_number_format($return['sale'], $decimals); // Giá bán thực tế của sản phẩm đã format
    $return['unit'] = $money_config[$currency_convert]['currency'];
//print_r($return);
    return $return;
}
