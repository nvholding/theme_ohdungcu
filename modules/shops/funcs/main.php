...
$array_id_item = $data_review = array();
...
if( empty( $contents ) )
{
    ...
    elseif( $pro_config['home_view'] == 'view_home_cat' )
    {
        foreach( $global_array_shops_cat as $catid_i => $array_info_i )
        {
            if( $array_info_i['parentid'] == 0 and $array_info_i['inhome'] != 0 )
            {
                ...
                $db->select( 'id, listcatid, publtime, ' . NV_LANG_DATA . '_title, ' . NV_LANG_DATA . '_alias, ' . NV_LANG_DATA . '_hometext, homeimgalt, homeimgfile, homeimgthumb, product_code, product_number, product_price, money_unit, discount_id, showprice, ' . NV_LANG_DATA . '_gift_content, gift_from, gift_to, discount_member_amount, discount_member_type, discount_num_pro, gift_info' )->order( 'id DESC' )->limit( $array_info_i['numlinks'] );

                $result = $db->query( $db->sql() );
                $data_pro = array();

                while( list( $id, $listcatid, $publtime, $title, $alias, $hometext, $homeimgalt, $homeimgfile, $homeimgthumb, $product_code, $product_number, $product_price, $money_unit, $discount_id, $showprice, $gift_content, $gift_from, $gift_to, $discount_member_amount, $discount_member_type, $discount_num_pro, $gift_info ) = $result->fetch( 3 ) )
                {
                    if( $homeimgthumb == 1 )
                    {
                        //image thumb
					...
					$discount_num_pro = $discount_num_pro ? json_decode($discount_num_pro, true) : array();
					$has_mydiscount = $discount_member_amount > 0 || (is_array($discount_num_pro) && count($discount_num_pro) > 0);
					
					$gift_info = $gift_info ? json_decode($gift_info, true) : array();
        			$has_gift_info = (is_array($gift_info) && count($gift_info) > 0);
    ...
    elseif( $pro_config['home_view'] == 'view_home_group' )
    {
        ...
        foreach( $global_array_group as $groupid_i => $array_info_i )
        {
            if( $array_info_i['parentid'] == 0 and $array_info_i['inhome'] != 0 )
            {
                ...
                // Fetch Limit
                $db->sqlreset()->select( 'DISTINCT id' )->from( $db_config['prefix'] . '_' . $module_data . '_rows t1' )->join( 'INNER JOIN ' . $db_config['prefix'] . '_' . $module_data . '_group_items t3 ON t3.pro_id = t1.id' )->where( $sql_regexp . ' AND t1.inhome=1 AND t1.status =1' );
    ...
    //rating data
    if( ! empty( $array_id_item ) )
    {
        $db->sqlreset()->select( 'rating, product_id' )->from( $db_config['prefix'] . '_' . $module_data . '_review t1' )->where( 't1.status = 1 AND t1.product_id IN (' . implode( ',', $array_id_item ) . ')' );
        $result = $db->query( $db->sql() );
        
        while ( list( $rating, $product_id ) = $result->fetch( 3 ) )
        {
            $data_review[$product_id][] = $rating;
        }
    }
    
    $contents = call_user_func( $pro_config['home_view'], $data_content, $compare_id, $html_pages, $sorts, $data_review );
