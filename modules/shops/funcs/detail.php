...
if (nv_user_in_groups($global_array_shops_cat[$catid]['groups_view'])) {
    ...
    if ($data_content['homeimgthumb'] == 1) {
     ...
     // Tu khoa
    $array_keyword = array();
    $key_words = ! empty( $key_words ) ? $key_words : array();
    ...
    $data_others = array();
    $array_id_item = array( $data_content['id'] );
    ...
    $contents = detail_product( $data_review, $data_content, $data_unit, $data_others, $array_other_view, $content_comment, $compare_id, $popup, $idtemplate, $array_keyword );
} else {
    $nv_redirect = NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name;
